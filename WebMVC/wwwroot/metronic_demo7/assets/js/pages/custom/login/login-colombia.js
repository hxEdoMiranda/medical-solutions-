...
var KTLoginGeneral = function () {
    ...
    var handleSignInFormSubmit = function () {
        $('#kt_login_signin_submit').click(function (e) {
            ...
            if (!(window.hasOwnProperty('host') ? window.host.includes("positiva") : false)) {
                form.validate({
                    rules: {
                        Username: {
                            required: true
            ...
            form.ajaxSubmit({
                url: (e.target.dataset.login) ? e.target.dataset.login : '/account/login',
                success: function (response, status, xhr, $form) {
                    if (response.msg == "error") {
                        if (window.hasOwnProperty('host') ? window.host.includes("positiva") : false) {
                            ...
                            Swal.fire({
                                type: "question",
                                text: "Si no te sientes bien, y no puedes ingresar, no te preocupes. Estamos aquí para ayudarte. Por favor, llámanos al (601)330-7000 ó 01-8000-111-170 y te atenderemos de inmediato. Tu bienestar es importante para nosotros.",
                                confirmButtonColor: '#FF7500',
                    ...
                    else if (response.msg == "loginVerify") {
                        if (window.hasOwnProperty('host') ? window.host.includes("positiva") : false) {
                            btn.removeClass('kt-spinner kt-spinner--right kt-spinner--sm kt-spinner--light').attr('disabled', false).removeAttr('style');
                            if (response.hasOwnProperty('loginVerify')) {
                                document.getElementById('loginNombre').removeAttribute("hidden");
                                document.getElementById('loginApellido').removeAttribute("hidden");
                                document.getElementById('loginCorreo').removeAttribute("hidden");
                                document.getElementById('loginVerify').value = "true";
                            }
                        }
                    }
                        ...
                        setTimeout(function () {
                            btn.removeClass('kt-spinner kt-spinner--right kt-spinner--sm kt-spinner--light').attr('disabled', false).removeAttr('style');
                            showErrorMsg(form, 'danger', 'Usuario inactivo, contactese con mesa de ayuda para solucionar este problema, envia un correo a contactocolombia@medismart.live o llamando al +57 313 2136751');
                        }, 1000);
                    }
