import { SaveOrder, CreateGuestCart, CreateCart, orderPaySuccessfuly, CreateInvoice, UpdateOrderPagoFarmazon, UpdateOrderStatus } from '../apis/carro-compras-fetch.js?rdn=';
import { enviarCorreoExamedi } from '../apis/correos-fetch.js';
import { putAgendarMedicosHoras, putAgendarMedicosHorasList, getAtencionByIdMedicoHora, confirmarEstadoAtencionWow, enviarConfirmacionWow } from '../apis/agendar-fetch.js?rnd=';
import { insertAtencionesInterconsultas, asociarAtencionesInterconsultas } from '../apis/atenciones-interconsultas-fetch.js';
...
var profesionalesInterconsultaSelected = {};
...
//dataresponse listaAgenda
let atencionesAgendadadas = []
...
export async function init(data) {
    ...
    $("#pagarButton").click(async function () {
        if (await botonesPagoValidar("soloInterc")) {
            
            let res = await putAgendarMedicosHorasList(ListaAgendas);
            console.log(res);
            let errMessage = res.find((r) => r.value.status === 'NOK');
            if (errMessage) {
                $("#loaderCar").modal("hide");
                $("#loaderCar").hide()
                $(".modal-backdrop.fade").hide();
                Swal.fire(errMessage.value.msg);
                return;
            }
            atencionesAgendadadas = res.map((v) => v.value)
            await confirmarAtencion();
            $("#loaderCar").modal("hide");
    ...
    async function botonesPagoValidar(pasarelaPago) {
        ...
        var valorTotalCar = $("#totalPriceI").text().substring(1, $("#totalPriceI").text().length);
        let medicamentos = $("#medicamentos-list input").is(":checked");
        let examenes = $("#examenes-list input").is(":checked");
        let interconsultaCheck = false
        let listCheckedinterconsulta = $("input[id^='chk-medico-interconsulta-tab_']");
        for (let checkedinterconsulta of listCheckedinterconsulta)
            if (checkedinterconsulta.checked)
                interconsultaCheck = true
        let medicoSeleccionado = ListaAgendas.length > 0;
        //let pagoTotalMsg = $("#totalPrice").text();
        let valorCeroInterconsultaExiste = 0;
        let soloPagarInterconsulta = 0;
        validarTelefonosPaciente();
        if (examenes == false && interconsultaCheck && ListaAgendas) { //Solo existe interconsulta
            soloPagarInterconsulta = 1;
        }
        for (var item of ListaAgendas) {
            if (item.valorAtencion == 0)
                valorCeroInterconsultaExiste += 1;
        };
        if (valorTotalCar >= 0) {
            if (soloPagarInterconsulta) {
                resInter = null;
...
//Funcion para confirmar atenciones. Esta se llama desde ...
async function confirmarAtencion() {
    for (let {infoAtencion} of atencionesAgendadadas) {
        let atencionId = infoAtencion.idAtencion;
        await asociarInterConsulta(atencionId);
        var atencionConfirmar = parseInt(atencionId);
        await confirmarEstadoAtencionWow(atencionConfirmar, 0);
        await enviarConfirmacionWow(atencionId);
    }
}
...
function findParentNode(element, search) {
    ...
    if (node.classList.contains("search")) {
        ...
        return findParentNode(node, search)
    }
}

function medicosActions(data) {
    if (data.especialidadesDerivacionPrograma && data.especialidadesDerivacionPrograma.length > 0)
        data.especialidadesDerivacionPrograma.forEach((e) => {
            profesionalesInterconsultaSelected[String.valueOf(e)] = { ischecked: false, agendaSelect: null }
        })
    let loadedInterconsultas = []
    if (data.interconsultas) {
        loadedInterconsultas = data.interconsultas.map((e) => ({ medicoHora: e.idMedicoHora, especialidad: e.horaMedico.codigoEspecialidad }));
        for (let inter of loadedInterconsultas) {
            let selector = $("#divContentInterconsulta select[id^='lista-medico-interconsulta-tab_'][data-especialidad='" + inter.especialidad + "']")
            let option = $("#divContentInterconsulta select[id^='lista-medico-interconsulta-tab_'][data-especialidad='" + inter.especialidad + "'] option[value=" + inter.medicoHora +"]")
            if (selector.length == 0 || option.length == 0)
                continue
            selector.val(inter.medicoHora)
            let selectedValues = selector[0].options[selector[0].selectedIndex].dataset
            if ($("#divContentInterconsultaDetails")) {
                let i = inter.especialidad
                let a = $("#divContentInterconsultaDetails div[id^='item-medico-interconsulta-tab_" + i + "']")[0];
                let check = $("#divContentInterconsultaDetails div[id^='item-medico-interconsulta-tab_" + i + "'] input[type='checkbox']")[0];
                let select = selector[0]
                check.checked = true
                check.disabled = true
                select.disabled = true
                select.style = "width: 65% !important;border-color: #fff0; -webkit - appearance: none;"
                //console.log(a)
                //a.getElementsByClassName("item-medico-name").item(0).innerText = selectedValues.namemedico
                //a.getElementsByClassName("item-medico-fecha").item(0).innerText = selectedValues.fecha
                //a.getElementsByClassName("item-medico-speciality").item(0).innerText = selectedValues.especialidad
                //a.getElementsByClassName("item-medico-hora").item(0).innerText = selectedValues.hora
                //a.getElementsByClassName("item-medico-price").item(0).innerText = selectedValues.price
                //getPreSaveInterconsulta($("#chk-medico-interconsulta")[0], data);
               /* newGetPreSaveInterconsulta({
                    idMedico: selectedValues.idmedico,
                    idMedicoHora: selectedValues.idmedicohora,
                    speciality: selectedValues.especialidadid,
                    checked: check.checked,
                    saved: true
                }, data)*/
                //$("#divContentInterconsultaDetails div[id^='item-medico-interconsulta-tab_" + i + "']").show()
                //$("#divContentInterconsulta input[id^='chk-medico-interconsulta-tab_" + i + "']")[0].checked = true
            }
        }
    }

    if ($("#divContentInterconsultaDetails")) { //ONCHANGE details check Medico
        let listado = $("#divContentInterconsultaDetails div[id^='item-medico-interconsulta-tab_']");
        for (let i of data.especialidadesDerivacionPrograma) {
            if ($("#divContentInterconsultaDetails div[id^='item-medico-interconsulta-tab_" + i + "'] input[type='checkbox']").length == 0)
                continue;
            let checkes = $("#divContentInterconsultaDetails div[id^='item-medico-interconsulta-tab_" + i + "'] input[type='checkbox']")[0];
            checkes.addEventListener("change", (e) => {
                let check = $("#divContentInterconsulta input[id^='chk-medico-interconsulta-tab_" + i + "']")[0];
                let selectedVal = $("#divContentInterconsulta select[id^='lista-medico-interconsulta-tab_" + i + "']")[0];
                let boxFather = $("#divContentInterconsultaDetails div[id^='item-medico-interconsulta-tab_" + i + "']")[0];
                
                
                let selectedValues = selectedVal.options[selectedVal.selectedIndex].dataset;
                if (e.target.checked) {
                    ...
                    if (e.target.checked != check.checked) {
                        check.checked = false
                        $("#divContentInterconsultaDetails div[id^='item-medico-interconsulta-tab_" + i + "']").hide()
                    }
                }
                newGetPreSaveInterconsulta({
                    idMedico: selectedValues.idmedico,
    ...
    if ($("#divContentInterconsulta")) { //ONCHANGE check MEDICO
        let listado = $("#divContentInterconsulta input[id^='chk-medico-interconsulta-tab_']");
        for (let i of data.especialidadesDerivacionPrograma) {
            if ($("#divContentInterconsulta input[id^='chk-medico-interconsulta-tab_" + i + "']").length == 0)
                continue;
            $("#divContentInterconsulta input[id^='chk-medico-interconsulta-tab_"+i+"']")[0].addEventListener("change", (e) => {
                console.log(e)
                let check = $("#divContentInterconsultaDetails div[id^='item-medico-interconsulta-tab_" + i + "'] input[type='checkbox']")[0];
                let selectedVal = $("#divContentInterconsulta select[id^='lista-medico-interconsulta-tab_" + i + "']")[0];
                let boxFatherCheck = $("#divContentInterconsultaDetails div[id^='item-medico-interconsulta-tab_" + i + "']")[0];
                let val = selectedVal.options[selectedVal.selectedIndex].value
                let selectedValues = selectedVal.options[selectedVal.selectedIndex].dataset;
                if (val == 0)
                ...
                if (selectedValues.idmedico != undefined && selectedValues.idmedicohora != undefined && selectedValues.especialidadid != undefined)
                newGetPreSaveInterconsulta({
                    idMedico: selectedValues.idmedico,
                    idMedicoHora: selectedValues.idmedicohora,
                    speciality: selectedValues.especialidadid,
    ...
    if ($("#divContentInterconsulta")) { //ONCHANGE MEDICO
        let listado = $("#divContentInterconsulta select[id^='lista-medico-interconsulta-tab_']")
        for (let i of data.especialidadesDerivacionPrograma) {
            if ($("#divContentInterconsulta select[id^='lista-medico-interconsulta-tab_" + i + "']").length == 0)
                continue;
            $("#divContentInterconsulta select[id^='lista-medico-interconsulta-tab_"+i+"']")[0].addEventListener("change",(e) => {
                let selectedValues = e.target.options[e.target.selectedIndex].dataset
                if ($("#divContentInterconsultaDetails")) {
                    let a = $("#divContentInterconsultaDetails div[id^='item-medico-interconsulta-tab_" + i + "']")[0];
                    let check = $("#divContentInterconsultaDetails div[id^='item-medico-interconsulta-tab_" + i + "'] input[type='checkbox']")[0];
                    check.checked = true
...
function newGetPreSaveInterconsulta({ idMedico, idMedicoHora, speciality, checked}, data) {
    let medico = data.horasInterconsultasEspecialidades.find(x => x.idMedico == parseInt(idMedico) && x.idMedicoHora == parseInt(idMedicoHora) && x.codigoEspecialidad == parseInt(speciality));
    if (medico) {
        let valorConsulta = 0;
        let medicoAgendar = {
            id: 0,
            ...
            idProgramaSalud: parseInt(data.atencion.idProgramaSalud),
            isProgramaSalud: true
        };
        if (profesionalesInterconsultaSelected) {
            profesionalesInterconsultaSelected[String(speciality)] = { ischecked: checked, agendaSelect: medicoAgendar }
            console.log(profesionalesInterconsultaSelected)
        }
        ListaAgendas = []
        for (let key of Object.keys(profesionalesInterconsultaSelected))
            if (profesionalesInterconsultaSelected[key] && profesionalesInterconsultaSelected[key].ischecked)
                ListaAgendas.push(profesionalesInterconsultaSelected[key].agendaSelect)
...
async function asociarInterConsulta(idInterconsulta) {
    await asociarAtencionesInterconsultas(modelVista.atencion.id, idInterconsulta);
}

async function insertarAtencionInterconsulta() {
    await insertAtencionesInterconsultas(modelVista.atencion.id, idInterconsulta);
}
...
async function redireccionCompraCustom() {
    location.href = `/programaSalud/CompraCustom?idAtencion=${modelVista.atencion.id}`;
}
