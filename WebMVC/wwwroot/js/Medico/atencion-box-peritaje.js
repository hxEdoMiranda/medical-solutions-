...
var checkMenoscabo = document.querySelector('[id="checkMenoscabo"]');
var checkAcoso = document.querySelector('[id="checkAcoso"]');
var checkMaltrato = document.querySelector('[id="checkMaltrato"]');
var checkDisfuncional = document.querySelector('[id="checkDisfuncional"]');
var checkHostigamiento = document.querySelector('[id="checkHostigamiento"]');
var checkHostilizacion = document.querySelector('[id="checkHostilizacion"]');
...
export async function init(data) {
    $(document).ready(async function () {
        ...
        if (peritaje) {
            ...
            //$('[name="DiagnosticoMedico"]').val(peritaje.anamnesis);
            ...
            $('#EJEI').val(peritaje.ejei);
            $('#EJEII').val(peritaje.ejeii);
            $('#EJEIII').val(peritaje.ejeiii);
            $('#EJEIV').val(peritaje.ejeiv);
            $('#EJEV').val(peritaje.ejev);
            ...
            $('#puntajePeritaje').val(peritaje.puntajePeritaje);
        }
    ...
    document.querySelector('#btnGuardar').onclick = async () => {
        ...
        let hisPsiquiatricoFamilia = $('[name="hisPsiquiatricoFamilia"]').val();
        ...
        if (checkistCausas.checked) {
            ...
            if (checkMenoscabo.checked) menoscabo = 1;
            else menoscabo = 0;

            if (checkAcoso.checked) acoso = 1;
            else acoso = 0;

            if (checkMaltrato.checked) maltrato = 1;
            else maltrato = 0;

            if (checkDisfuncional.checked) disfuncional = 1;
            else disfuncional = 0;

            if (checkHostigamiento.checked) hostigamiento = 1;
            else hostigamiento = 0;

            if (checkHostilizacion.checked) hostilizacion = 1;
            else hostilizacion = 0;
        ...
        if (((hisMorbido == "" || hisQuirurgica == "" || hisAlergias == "" || hisHabitoso == "")&& !check.checked && data.atencion.horaMedico.idConvenio == 42)) {
        ...
        let formPeritaje = {
            ...
            menoscaboLaboral: menoscabo,
            acosoLaboral: acoso,
            maltratoLaboral: maltrato,
            disfuncionalLaboral: disfuncional,
            hostigamientoLaboral: hostigamiento,
            hostilizacionLaboral: hostilizacion,
            ...
            hisPsiquiatricoFamilia: hisPsiquiatricoFamilia,
        ...
        async function guardar() {
            ...
            if (valida.status == "OK") {
                if (!check.checked && data.atencion.peritaje) {
                    ...
                    document.getElementById("historiahisPsiquiatricoFamilia").innerHTML = peritaje.hisPsiquiatricoFamilia;
                    ...
                    document.getElementById("ejeiInforme").innerHTML = peritaje.ejei;
                    document.getElementById("ejeiiInforme").innerHTML = peritaje.ejeii;
                    document.getElementById("ejeiiiInforme").innerHTML = peritaje.ejeiii;
                    document.getElementById("ejeivInforme").innerHTML = peritaje.ejeiv;
                    document.getElementById("ejevInforme").innerHTML = peritaje.ejev;
    ...
    $('#form_edit_peritaje').validate({
        ...
        submitHandler: async function (form, e) {
            ...
            let puntajePeritaje = parseInt($('#puntajePeritaje').val());
            ...
            if (isNaN(puntajePeritaje) || puntajePeritaje == "") {
                Swal.fire("¡Cuidado! <br>Completar puntaje peritaje <br>", "", "error");
                return;
            }
            ...
            let formPeritaje = {
                ...
                PuntajePeritaje: puntajePeritaje,
...
const eje5 = document.getElementById('EJEV');
...
eje5.onblur = function () {
    var puntos = eje5.value ?? "";

    if (puntos == "") {
        document.getElementById("eje5Date").innerHTML = "";
        return;
    }
    ...
    var res = puntos.replace(/[^0-9]+/g, "");
    res = parseInt(res);

    var dias = puntajes.get(res);
    const dateControl = new Date();
    dateControl.setDate(dateControl.getDate() + dias);   
    
    var diasAPP = dateControl.getFullYear() + "-" + padNumber(dateControl.getMonth() + 1) + "-" + padNumber(dateControl.getDate());
    document.getElementById("eje5Date").innerHTML = "Días app de termino de licencia o recargo de dias : " + diasAPP;
}
