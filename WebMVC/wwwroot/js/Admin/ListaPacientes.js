import { GetListaPacientes, ChangeStatePersona } from "../apis/personas-fetch.js";
...
export async function init(data, urlEdit) {
    ...
    btnBuscar.onclick = async () => {
        ...
        if(datatable!=null)
            datatable.destroy();
        let pacientes = await GetListaPacientes(textBusqueda.value, selectEstado.value);
            ...
            columns: [
                ...
                },
                {
                    ...
                    template: function (row) {
                        if (row.estado == "I") {
                            return '\
                        <a href="/Admin/AgendaPaciente/'+ row.userId + '" class="btn btn-sm btn-clean btn-icon btn-icon-md" id="reserva'+row.userId+'" title="Reservar Hora" hidden>\
                       ...
                       <button class="btn btn-sm btn-clean btn-icon btn-icon-md openModal" id="btnEditarAgenda'+ row.userId + '" data-userid="' + row.userId + '" hidden title="Editar Agenda">\
                            ...
                            return '\
                        <a href="/Admin/AgendaPaciente?idPaciente='+ row.userId + '" class="btn btn-sm btn-clean btn-icon btn-icon-md" id="reserva' + row.userId +'" title="Reservar Hora">\
							<i class="far fa-calendar-plus"></i>\
						</a>\
        ...
        columns: [
            ...
            },
            {
                ...
                template: function (row) {
                        ...
                        <button class="btn btn-sm btn-clean btn-icon btn-icon-md eliminarAtencion" data-fecha="'+row.fecha+'" data-idcliente= "'+row.idCliente+'" data-idmedico="'+ row.idMedico+ '" data-idatencion="'+ row.idAtencion + '" title="Inactivo">\
                ...
                if (idCliente == 1) {
                    ...
                    if (validar != "NOK") {
                        if (!validar.reschedule && validar.id_agenda == 0) {
                            Swal.fire("No es posible reagendar esta atención Paciente producto plus", "Indicar a usuario que debe cancelar atención por sucursal digital.", "error");
