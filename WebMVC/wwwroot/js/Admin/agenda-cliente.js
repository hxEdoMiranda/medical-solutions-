...
import { getVwHorasMedicosByMedic, getVwHorasMedicosBloquesHorasByMedic, getHoraMedico, getVwHorasMedicoByAtencion } from '../apis/vwhorasmedicos-fetch.js';
import { getMedicosbyEspecialidad, getMedicos, putAgendarMedicosHoras, reagendarApp, revertReagenda } from '../apis/agendar-fetch.js?5';
import { comprobantePaciente, comprobanteMedico, comprobanteProfesionalAsociados } from '../apis/correos-fetch.js';
import { getDestinatariosAsociados } from "../apis/atencion-fetch.js";
...
export async function init(data) {
    ...
    if (idAtencion != 0) {
        ...
        var fecha;
        let dataAtencion = await getVwHorasMedicoByAtencion(idAtencion);
       ...
       if (result.value) {
            ...
            switch (cliente) {
                case 1://flujo reagendar consalud
                    debugger
                    if (peritaje) {
                        agendar.idCliente = cliente ? cliente : 0;
                        if (idAtencion == 0) {
                            valida = await putAgendarMedicosHoras(agendar, idMedico, uid);
                        }
                        else {
                            valida = await reagendarApp(agendar, idMedico, uid);
                        }
                    }
                    else {
                        ...
                        if (validar != "NOK") {
                            if (!validar.reschedule && validar.id_agenda == 0) {
                                Swal.fire("No es posible reagendar esta atención Paciente producto plus", "Indicar a usuario que debe reagendar atención por sucursal digital.", "error");
                                return;
                            }
            ...
            if (valida !== 0) {
               ...
               if (connection.state === signalR.HubConnectionState.Connected) {
                ...
                if (typeAgenda != "E") {
                    if (!host.hostname.includes('unabactiva.') &&
                        !host.hostname.includes('activa.unab.') &&
                        !host.hostname.includes('achs.') &&
                        !host.hostname.includes('uoh')) {
                ...
                location.href = `/Admin/EditaAgenda?idAtencion=${valida.infoAtencion.idAtencion}&idPaciente=${valida.infoAtencion.idPaciente}`;
            }
            else {
