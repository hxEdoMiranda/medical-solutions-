...
export async function init(data) {
    ...
    var datos = await personByUser(uid);
   ...
   //document.getElementById("valorAtencion").innerHTML = formatNumber(valorAtencion).toLocaleString();
    ...
    buttonConfirmar.addEventListener("click", async function (event) {
            ...
            var url = "/PacienteInvitado/ConfirmarAtencion" + "?idMedicoHora=" + idMedicoHora + "&idMedico=" + idMedico + "&idBloqueHora=" + idBloqueHora + "&fechaSeleccion=" + fecha + "&hora=" + horaText + "&horario=" + horario + "&idAtencion=" + idAtencion + "&m=" + m + "&r=" + r + "&c=" + c;
            ...
            if (valida.status === "NOK") {
                ...
                if (hashBono) {
                    ...
                    if (validaAtencionRepetida.status == "EXISTE") {
                        ...
                        var responseJson = await PostPagoInvitado(hashBono, document.getElementById('correoUsuario').value, agenda.especialidad, valorAtencion);
                        if (responseJson)
