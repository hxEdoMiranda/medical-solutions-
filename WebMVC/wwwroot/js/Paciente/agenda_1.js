...
import { getBuscaExcepcionesDoc } from '../apis/personas-fetch.js';
var numDoc;
...
export async function init(data, urlPaso1) {
    // MENSAJE ESPECIALIDAD PEDIATRIA
    if (especialidad == 4) {
        $("#modalAlertaPediatria").modal("show");
    }
    ...
    var validaDoc = await getBuscaExcepcionesDoc(numeroDocumento);      
    ...
    async function loadEdit(event) {
        ...
        if (document.getElementById('aceptaTerminos').checked) {
            ...
            if (fNacimiento && fNacimiento.value.length < 10) {
                Swal.fire(
                    '',
                    'Debes completar la información de nacimiento en tu perfil para continuar.',
                    'warning'
                )
                return;
            }
            ...
            /*VALIDACION DE EXCEPCION DE DOCUMENTO*/
            var validaDoc = await getBuscaExcepcionesDoc(numeroDocumento);
